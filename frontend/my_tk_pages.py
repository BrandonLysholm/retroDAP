...
class CloseOpenboxFrame(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        
        self.configure(bg=SPOT_BLACK)
        self.header_label = tk.Label(self, text ="Close Openbox", font = LARGEFONT, background=SPOT_BLACK, foreground=SPOT_GREEN) 
        self.header_label.grid(sticky='we', padx=(0, 10))
        self.grid_columnconfigure(0, weight=1)
        divider = tk.Canvas(self)
        divider.configure(bg=SPOT_GREEN, height=DIVIDER_HEIGHT, bd=0, highlightthickness=0, relief='ridge')
...
class WiFiPageFrame(tk.Frame):
    ...
    def update_wifi_input(self, input):
        if (input == "ssid"):
            self.ssid_letter_label.configure(text="a", background=SPOT_GREEN, foreground=SPOT_BLACK)
            self.pw_letter_label.configure(text="", background=SPOT_BLACK, foreground=SPOT_GREEN)
        elif (input == "pw"):
...
class USBPassthroughFrame(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        ...
        contentFrame.grid_columnconfigure(1, weight=1)
        self.grid_rowconfigure(2, weight=1)
        self.confirmation_label = tk.Label(contentFrame, text ="To enter USB passthrough, flip the holdswitch before plugging into a computer", font = MED_FONT, background=SPOT_BLACK, foreground=SPOT_GREEN, wraplength=600)
        self.confirmation_label.grid(row=0, column=0,sticky ="nswe", padx=(0,10))
